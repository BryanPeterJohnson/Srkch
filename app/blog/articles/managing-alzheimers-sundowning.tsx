...
export default function PostStrokeNutritionContent() {
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-[#003A5C] mt-10 mb-4 tracking-tight">5. Micronutrient Synthesis for Myelin Sheath Repair</h2>
      <p className="text-slate-600 leading-relaxed mb-6">
        Axonal rewiring depends heavily on the thickness and integrity of myelin insulation. Vitamin B12 (cobalamin), folate, and pyridoxine ($Vitamin\ B_6$) act as required cofactors in methylation pathways that actively rebuild these lipid sheaths.
      </p>
      <p className="text-slate-600 leading-relaxed mb-6">
        Deficiencies in these core vitamins can result in elevated systemic homocysteine levels—a recognized biomarker for vascular damage. Monitoring these micro-metrics through targeted routine blood panels and adjusting dietary delivery accelerates macro-recovery timelines.
      </p>

      <h2 className="text-2xl font-bold text-[#003A5C] mt-10 mb-4 tracking-tight">6. Mitigating Insulin Resistance and Glycemic Spikes</h2>
      <p className="text-slate-600 leading-relaxed mb-6">
        Fluctuations in blood glucose levels post-stroke can amplify the size of the neural lesion by exacerbating oxidative stress within tissue margins. Traditional high-glycemic carbohydrates cause insulin spikes that worsen inflammatory pathways and limit endothelial cell recovery.
      </p>
      <p className="text-slate-600 leading-relaxed mb-6">
        Transitioning meal designs toward low-glycemic, complex carbohydrates paired with clean proteins keeps systemic glucose curves flat. This stable metabolic baseline reduces vascular stress, protecting fragile cerebral tissues from secondary ischemic events.
      </p>
      ...
      <div className="my-8 p-5 rounded-2xl bg-slate-50 border border-slate-200/60 text-slate-600 text-sm">
        ...
        <ul className="list-disc pl-5 space-y-2 text-xs md:text-sm">
          ...
          <li><strong>Glycemic Index Caps:</strong> Limit single-meal carbohydrate loads to complex, high-fiber grains to avoid glucose spikes.</li>
          <li><strong>B-Complex Supplementation:</strong> Monitor and maintain optimal B12 and Folate counts to optimize myelin repair.</li>
        </ul>
      </div>
    </>
  );
